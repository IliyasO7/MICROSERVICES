...
exports.verifyLoginOtp = async (req, res, next) => {
  try {
    ...
    // Generate token
    let token = await jwt.sign({ customerId: customer._id, role: 'customer' }, process.env.JWT_KEY, { expiresIn: '6h' })
...
exports.profile = async (req, res, next) => {
  try {
    ...
    let profile = await Customer.findById(req.userData.customerId, '_id fname lname email phone addresses service country status').populate('service', 'name description pricing ').lean()
...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    await Customer.updateOne({ _id: req.userData.customerId }, {
      $set: {
        'fname': req.body.fname,
        'lname': req.body.lname,
        'email': req.body.email,
...
exports.updatePassword = async (req, res, next) => {
  try {
    ...
    let updatePassword = await customersService.updatePassword(req.userData.customerId, req.body.password)
