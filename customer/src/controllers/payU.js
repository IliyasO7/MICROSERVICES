import jsSHA from 'jssha';
  ...
  export const payUMoneyPayment = async (req, res) => {
    var pd = req.body;
    var hashString = process.env.PAYUMERCHANTKEY // Merchant Key 
             + '|' + pd.txnid 
  ...
  export const payUResponse = async (req, res) => {
    ...
    //Generate new Hash 
     var hashString = process.env.PAYUSALT + '|' + pd.status + '||||||||||' + '|' + pd.email + '|' + pd.firstname + '|' + pd.productinfo + '|' + pd.amount + '|' + pd.txnid + '|' + process.env.PAYUMERCHANTKEY
     var sha = new jsSHA('SHA-512', "TEXT");
     sha.update(hashString)
     var hash = sha.getHash("HEX");
