...
export const createSuperAdmin = async (req, res) => {
  ...
  if (saveUser)
    sendResponse(res, 200, "Super Admin Created Successfully", { saveUser });
};
...
export const createOdsAdmin = async (req, res) => {
  ...
  if (saveUser)
    sendResponse(res, 200, "Ods Admin Created Successfully", { saveUser });
};
...
export const createRentalAdmin = async (req, res) => {
  ...
  if (saveUser)
    sendResponse(res, 200, "Rental Admin Created Successfully", { saveUser });
};
...
export const getTenant = async (req, res) => {
  ...
  if (user) {
    ...
    let transactions = await RentalTransactions.find({ from: user }).populate(
      "to"
    );
    if (tenant) {
      return sendResponse(res, 200, "Tenant Record Found Pls Udpate", {
        tenant,
        booking,
        transactions,
      });
...
*/
  ...
  sendResponse(res, 200, "Tenant And Booking Added successfully", {
    saveUser,
    createUserAsTenant,
  ...
  if (!tenantUser) {
    ...
    if (!prevBooking) {
      ...
      const paidUntil = new Date(
        `${paidYear}-${nextMonth}-${paidFromStartDate}`
      );
      console.log(paidUntil);
...
export const getOwner = async (req, res) => {
  ...
  if (user) {
    ...
    let transactions = await RentalTransactions.find({ to: user }).populate(
      "from"
    );

    if (owner) {
      return sendResponse(res, 200, "Owner Record Found Pls Udpate", {
        owner,
        inventory,
        booking,
        transactions,
...
export const getInventoryDetails = async (req, res) => {
  ...
  let allInventories = await Inventory.find({ createdBy: userId }).populate(
    "user"
  );
  return sendResponse(res, 200, "Inventories Fetched Successfully", {
    allInventories,
  });
};
...
export const getInventorywithId = async (req, res) => {
  ...
  return sendResponse(res, 200, "Inventory Data Fetched Successfully", {
    inventory,
  });
};
...
export const getAllInventoryDetails = async (req, res) => {
  ...
  return sendResponse(res, 200, "All Inventories Fetched Successfully", {
    allInventoriesData,
  });
};
...
export const getOwnerInventory = async (req, res) => {
  ...
  let allInventoriesData = await Inventory.find({ user: user }).populate(
    "user"
  );
  return sendResponse(res, 200, "All Owner Inventories Fetched Successfully", {
    allInventoriesData,
  });
};
...
export const getBookingDetails = async (req, res) => {
  let allBookings = await Booking.find({ bookingId: req.params.bookingId })
    .populate("inventory")
    .populate("owner")
    .populate("tenant");
  return sendResponse(res, 200, "Bookings Fetched Successfully", {
...
export const getAdminTenants = async (req, res) => {
  const userId = req.user;
  let tenants = await RentalTenant.find({ createdBy: userId })
    .populate("user")
    .populate("createdBy");
  return sendResponse(res, 200, "Inventories Fetched Successfully", {
    tenants,
...
export const getAllowners = async (req, res) => {
  ...
  return sendResponse(res, 200, "All Owners Fetched Successfully", {
    allOwners,
  });
};
    ...
    if (req.files.mainImage) {
      ...
      for (let i = 0; i < req.files.mainImage.length; i++) {
        ...
        if (mainImages.status === 201) {
          data.mainImage[
            i
          ] = `https://housejoy.b-cdn.net/owner/inventory/main/${inventory._id}-${req.files.mainImage[i].originalname}`;
        }
        await Inventory.updateOne(
          { _id: inventory._id },
          {
    ...
    if (req.files.entranceImage) {
      for (let i = 0; i < req.files.entranceImage.length; i++) {
        ...
        if (entranceImages.status === 201) {
          data.entranceImage[
            i
          ] = `https://housejoy.b-cdn.net/owner/inventory/entrance/${inventory._id}-${req.files.entranceImage[i].originalname}`;
        }
        await Inventory.updateOne(
          { _id: inventory._id },
          {
    ...
    if (req.files.livingImage) {
      for (let i = 0; i < req.files.livingImage.length; i++) {
        ...
        if (livingImages.status === 201) {
          data.livingImage[
            i
          ] = `https://housejoy.b-cdn.net/owner/inventory/living/${inventory._id}-${req.files.livingImage[i].originalname}`;
        }
        await Inventory.updateOne(
          { _id: inventory._id },
          {
    ...
    if (req.files.bedroomImage) {
      for (let i = 0; i < req.files.bedroomImage.length; i++) {
        ...
        if (bedroomImages.status === 201) {
          data.bedroomImage[
            i
          ] = `https://housejoy.b-cdn.net/owner/inventory/bedroom/${inventory._id}-${req.files.bedroomImage[i].originalname}`;
        }
        await Inventory.updateOne(
          { _id: inventory._id },
          {
    ...
    if (req.files.kitchenImage) {
      for (let i = 0; i < req.files.kitchenImage.length; i++) {
        ...
        if (kitchenImages.status === 201) {
          data.kitchenImage[
            i
          ] = `https://housejoy.b-cdn.net/owner/inventory/kitchen/${inventory._id}-${req.files.kitchenImage[i].originalname}`;
        }
